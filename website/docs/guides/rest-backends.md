...
registers. No native code, no new package, no waiting — and no setup: the runtime calls
`AddBlazorNativeHttp()` itself before your `ConfigureServices` runs, so a component can inject
