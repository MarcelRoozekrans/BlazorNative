 ...
 *
 * The test drives the host root's [android.view.View.layout] directly (via
 * [SyntheticHost.resize]) — that is the exact call the framework makes on a real
 * resize, and it is deterministic, where rotating the AVD under instrumentation is
 * not. The host root is a [BnYogaFrameLayout], the PRODUCTION container.
 *
...
class YogaResizeAndroidTest {
    ...
    @Test fun frames_recompute_when_the_host_root_is_resized() {
        val d = density()
        val host = SyntheticHost(widthDp = 200f, heightDp = 400f)
        host.render(listOf(
        ...
        host.resize(widthDp = 400f, heightDp = 300f)
