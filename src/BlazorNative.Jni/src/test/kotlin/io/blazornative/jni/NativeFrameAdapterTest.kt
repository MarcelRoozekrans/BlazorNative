 ...
 *  3. golden_mountHello_viaNativeDll_matchesJsonPath — the Gate 2 exit test:
 *     mount(HelloComponent) inside the NativeAOT dll fires the JNA frame
 *     callback, and the adapter's output equals the committed JSON-path golden
 *     fixture (src/test/resources/hello-frame.json, generated by
...
class NativeFrameAdapterTest {
    ...
    // ── 3. Golden: native dll path == JSON path ──────────────────────────────

    @Test
    fun golden_mountHello_viaNativeDll_matchesJsonPath() {
        initNativeHost()
        ...
        // JSON path: the committed golden fixture (already frameId/timestamp-
        // normalized by HelloGoldenTests). FrameStreamParser wants the [FRAME]
        // stdout framing, so prepend the prefix rather than duplicating its
        // Json config here.
        val fixtureJson = javaClass.getResource("/hello-frame.json")!!.readText().trim()
        val golden = FrameStreamParser.parse("[FRAME] $fixtureJson").single()

        assertEquals(
            normalize(golden.patches),
            normalize(nativeFrame!!.patches),
            "native-callback patch list must equal the JSON path's golden patch list"
        )
    }
