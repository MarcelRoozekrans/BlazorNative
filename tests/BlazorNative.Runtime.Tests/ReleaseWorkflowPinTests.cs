...
// release pipeline's two normative rules, pinned rather than reviewed).
//
...
//
// WHY THESE TWO FACTS ARE TESTS AND NOT COMMENTS. `release.yml` fires ONCE PER
// RELEASE — it is the least-exercised code in the repository, and both of these
// rules fail SILENTLY and EXPENSIVELY:
//
//   1. SECRET CONTAINMENT — nothing publishes except a published Release, and
//      the mechanism of that rule is that `secrets.NUGET_API_KEY` is reachable
//      from EXACTLY ONE job in EXACTLY ONE workflow. A second reference — a
//      push step bolted into ci.yml, a "convenience" key in the validate job —
//      would move the door without moving the sign on it, and no lane would
//      notice. There is no review that catches this reliably; there is a test.
//
...
//      (AdoNet.Async's release.yml: `VERSION="${TAG#v}"` -> `pack
//      -p:PackageVersion=$VERSION`), which can afford it because GitVersion
...
// wrong reason — a deleted, renamed, moved or emptied release.yml would
...
public sealed class ReleaseWorkflowPinTests
{
    ...
    /// in the presence of block scalars (release.yml's `run: |` steps contain
    /// literal `#` characters that are CONTENT, not comments) — so the parser
    /// would either be fooled or, worse, eat its own subject and go green. A
    /// pin that occasionally reds on prose fails LOUD and takes ten seconds to
    /// fix; a pin with a parser that can blind itself fails SILENT.
    ///
    /// This is not theoretical: the pin's first run reddened on release.yml's
    /// own `env:` comment, which quoted the expression it was describing. The
    /// comment was reworded to name the expression without its braces.</summary>
    ...
    /// release.yml. Mutation: add the reference to ci.yml -> red NAMING the
    /// file.</summary>
    [Fact]
    public void TheNugetApiKey_IsReferencedExactlyOnce_AndOnlyInReleaseYml()
    {
        ...
        Assert.True(
            total == 1 && referencesByFile.Count == 1 && referencesByFile[0].File == ReleaseWorkflow,
            "NOTHING PUBLISHES EXCEPT A PUBLISHED GITHUB RELEASE (8.2 normative rule 1), and the "
            + "mechanism of that rule is that ${{ secrets.NUGET_API_KEY }} is reachable from "
            + $"EXACTLY ONE job in EXACTLY ONE workflow — `{ReleaseWorkflow}`'s `push` job, which "
            + "is guarded on `github.event_name == 'release'`.\n"
            + $"  expected: 1 reference, in {ReleaseWorkflow}\n"
            ...
            + "ci.yml would publish from a MERGE, which is verbatim what DoD #3 forbids. ZERO "
            + "references means the push job can no longer authenticate — or that this pin has "
            + "gone blind. Neither is a thing to green by editing this test.");
    }
    ...
    /// `-p:Version=1.2.3` to release.yml -> red.</summary>
    [Fact]
    public void TheReleaseWorkflow_NeverOverridesTheVersion()
    {
        string source = ReadReleaseWorkflow();
        ...
        Assert.True(
            ...
            $"could not find `dotnet nuget push` in {ReleaseWorkflow} — the release path moved or "
        ...
        Assert.True(offenders.Count == 0,
            ...
            + $"`-p:PackageVersion=` may appear in {ReleaseWorkflow}: overriding the version there "
            ...
            + "and never overrides it. Bump the props in a PR, then tag the merge commit.");
    }
    ...
    /// <summary>release.yml's text, with existence AND non-emptiness asserted.
    /// An absent or empty file satisfies every absence claim in this class
    /// perfectly — which is why it is a RED here, not a pass.</summary>
    private static string ReadReleaseWorkflow()
    {
        string file = CheckoutPath(ReleaseWorkflow);
        Assert.True(File.Exists(file),
            $"{ReleaseWorkflow} not found at {file} — it is THE ONE DOOR (8.2 design). Its absence "
            + "must be a RED, not a vacuous pass over a missing read.");

        string source = File.ReadAllText(file);
        Assert.False(string.IsNullOrWhiteSpace(source),
            $"{ReleaseWorkflow} is EMPTY — every absence assertion in this class would pass over "
