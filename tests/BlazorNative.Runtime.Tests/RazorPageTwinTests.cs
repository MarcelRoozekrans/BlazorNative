...
public sealed class RazorPageTwinTests
{
    ...
    // ── BnScrollDemo (Gate 3 — declarative; the @for loop with per-row content) ─

    /// <summary>Every patch of every frame — mount (the 20-create tree whose
    /// styles, insert indices and single `src` prop BnScrollDemoTests pins,
    /// and whose content-size arithmetic both shells assert) AND the "← Back"
    /// navigation (this page's removes + BnDemo's creates) — byte-identical
    /// between the hand-written BnScrollDemo and BnScrollDemoRazor. Covers
    /// the @for loop whose CHILD CONTENT differs per row (row 0's image, row
    /// 1's nested flex row, eight childless bands): the .razor gives EVERY
    /// row a ChildContent fragment where the .cs gave only rows 0 and 1 one —
    /// an empty fragment must be wire-invisible, and this is the proof.
    /// Mutation-verified (Gate 1 discipline) — see the phase conclusion's
    /// table.</summary>
    [Fact]
    public void BnScrollDemo_GoldenVsTwin_MountAndBack_Identical()
    {
        List<RenderPatch[]> Run(Func<NativeRenderer, int> mount)
            => Drive("BnScrollDemo", mount, m =>
                Assert.Equal(0, Exports.DispatchEventCore(
                    (ulong)ClickHandlerOnText(m, "← Back"), ClickArgs)));

        List<RenderPatch[]> golden = Run(r => r.Mount<BnScrollDemo>());
        List<RenderPatch[]> razor = Run(r => r.Mount<BnScrollDemoRazor>());
