...
|---|---|---|---|---|
| 1 | **`SetStyle` patch ORDER changes.** Patches are name-keyed but emitted in frame-array order, which follows sequence numbers. Layout is unaffected — Yoga does not care what order styles arrive in — but order-dependent assertions would red for a benign reason. | `.NET` component tests | **Medium** *(downgraded from High, 2026-08-19)* | **Downgraded on evidence found while writing the plan:** the existing golden helpers are already name-keyed — `StylesOf(frame, nodeId)` returns a `Dictionary<string, string?>`, and `StyleOn` filters on `p.Property == prop`. Both are order-independent by construction, so the dominant assertion style is already immune. Residual risk is any assertion that indexes `Patches` positionally. Group 1's patch-set capture still runs — it converts "already immune" from a reading into a measurement — but this is not the phase's leading hazard. Fix any order-dependent assertion by making it order-independent, **never** by loosening what it asserts. |
