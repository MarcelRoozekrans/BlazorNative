...
- ✅ **Phase 2.4** — Render-frame consumer (WASM-side dispatch + Android-side parse) — *complete (2026-05-27)*
   - **Partial-GREEN — 2 of 3 runtimes pass.** Same `.wasm` mounts the `_BridgeFrameSelfTest` sentinel in `Main`, emits one `[FRAME] {json}` line via sync `Console.WriteLine`; host parses by line-prefix, deserializes via `kotlinx.serialization` sealed `RenderPatch` hierarchy keyed on `op` discriminator; `handlers.onFrame` fires with the parsed frame. **Wasmtime CLI subprocess (.NET-side `BootSmoke` + `FrameSelfTestParsesAsRenderFrame`): PASS.** **JVM in-process JNA (`BootSmokeTest` + `FrameDispatchTest`): PASS.** **Android in-process JNA (`BootSmokeAndroidTest`): FAIL** — wasmtime v45 panic_bounds_check in `Func::call → post_return_impl → export_lifted_function` during/after `wasi:cli/run.run` returns. Carryover: investigate wasmtime upgrade or cargo-build flags. Matches Phase 2.2's `wasmtime_component_linker_delete` SIGABRT pattern — Android-only known issue.
...
- ⏳ **Phase 2.4b** — Android wasmtime regression unblocker — *pending (carried over from Phase 2.4)*
   - Wasmtime v45's `Func::call → post_return_impl → export_lifted_function` panics with `panic_bounds_check` on Android x86_64 after Phase 2.4 enlarged the reachable Mono-AOT IL surface (Mount<T> + BlazorInterop + DispatchFrame + sentinel). Phase 2.2 + 2.3's Android tests passed with the smaller .wasm. Investigation targets: (a) bump wasmtime to v46+ and re-cross-compile via `cargo ndk`; (b) review wasmtime cargo build flags for component-model + Android-ABI feature combinations; (c) if neither works, open an upstream issue with the minimal-repro wasm produced by Phase 2.4.
