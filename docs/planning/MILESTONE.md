...
2. **Bidirectional event flow.** `<button @onclick>` round-trips end-to-end: tap on Android widget → host invokes .NET event-dispatch C-ABI export → the Blazor component's handler fires → re-render emits a frame → widget tree updates. **Long-running-Main concern retires automatically** under the Phase 3.0 NativeAOT decision — the runtime library is always loaded, event dispatch is just another `[UnmanagedCallersOnly]` call. Implementation lands in Phase 3.2. ABI groundwork shipped in 3.0d (`blazornative_dispatch_event` export + AttachEvent/DetachEvent wire slots, dormant); 3.2 wires Android touch events.
