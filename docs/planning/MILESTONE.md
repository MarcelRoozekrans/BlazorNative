...
**Status:** complete — closed 2026-07-10, tagged `v3.0` ([final audit](../plans/2026-07-10-milestone-3-final-audit.md): **PASS, all 11 DoD criteria**)
**Started:** 2026-05-28
...
7. **Navigation service** (`BlazorNative.Navigation`) — at minimum, `INavigationManager.NavigateTo(route)` triggers a root-component swap. Wired through the `shell_navigate` / `shell_current_route` exports from DoD #3. *Note (2026-07-09): the C-ABI layer for this now exists — Phase 3.1's `NativeShellBridge.NavigateAsync` / `GetCurrentRouteAsync` round-trip through the host's Navigate/CurrentRoute callbacks (route is an in-memory var + log on Android for now); this DoD item owns the actual navigation UI/root-component swap.* ✅ **CLOSED 2026-07-10 (Phase 3.5):** `INavigationManager.NavigateToAsync` triggers a real root-component swap **on-device** — the shipped demo is a two-page app (`/` → `BnDemo`, `/settings` → `BnSettingsPage`; tap "Settings →" → the whole screen swaps, "← Back" → BnDemo remounts fresh). Wired through the DoD #3 surface exactly as specified: host-notify via the `Navigate` callback; startup route resolved from the host's `CurrentRoute` buffer (first mount of the routed default only). Swap = `RemoveRootComponent` over the 3.3 disposal machinery → fresh `TryMount` → afterSwap route state; mid-dispatch swaps defer via `RunAfterDispatch` and drain before the export returns. **Honest wording:** the contract ships in Core (`INavigationManager`) with the impl in Runtime — the `BlazorNative.Navigation` *package* lift is M6 (packaging); host-initiated navigation (back button, deep links) is M5. See [Phase 3.5 conclusion](../plans/2026-07-10-phase-3.5-conclusion.md).
...
11. **Decision log committed.** Same pattern as M1/M2: design + plan doc per phase, plus an M3 final-audit doc at close. ✅ **CLOSED 2026-07-10 (Phase 3.5):** design + implementation plan per phase (3.0 through 3.5), a conclusion doc per phase — including two honest reversals recorded in place (3.0b's Gate-4 RED → dual-runtime pivot → superseded by 3.0c; 3.3's Region-drop diagnosis corrected by 3.4) — and the [M3 final audit](../plans/2026-07-10-milestone-3-final-audit.md) (all 11 criteria walked with evidence; scaffolding ledger: probe exports GONE — eight-export C-ABI; `TrimValidationProbes` + `CompositionProbe` STAY).

## Closure

**M3 closed 2026-07-10** at Phase 3.5 Gate 5. Audit verdict: **PASS — all 11 DoD criteria** ([final audit](../plans/2026-07-10-milestone-3-final-audit.md)), with per-criterion honesty notes on #3 (callbacks, not exports — direction inverted under NativeAOT), #5 (`@bind` mechanics closed; Razor syntax = M6), #6 (closed on evidence after the corrected Region diagnosis), #7 (behavior closed; `BlazorNative.Navigation` package lift = M6). Final counts: .NET 177/2, JVM 32, Android instrumented 32 — all strict. Tag `v3.0` on the merge commit on `main`. M4 (P3 — Production-Shippable) is next; carryover triage table in the audit.
