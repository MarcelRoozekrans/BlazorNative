...
| ~~**Binary break for existing consumers.** Moving members to a base is source-compatible but not binary-compatible.~~ **RISK WITHDRAWN 2026-08-20 — it was never real.** | ~~Anyone compiled against 0.10.0 must recompile.~~ **Callers are unaffected.** | **Measured, not reasoned:** a probe compiled against the pre-refactor assembly, memberrefs verified in IL, then the *same binary* run against old and new — both exit 0, identical values. Per ECMA-335 II.22.25 a `MemberRef` with a `TypeRef` parent resolves by walking the base chain, and moving a member **up** its hierarchy is explicitly non-breaking in dotnet/runtime's rules (**down** is the breaking direction). What actually changes is **reflection about declarations** (`DeclaredOnly` on `BnView`: 24 → 2). The real consumer-facing effect is narrow: an XML cref naming the old declaring type raises **CS1574, a warning**, only in doc-generating projects. |
